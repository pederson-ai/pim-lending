...
export function PaymentForm({ loanId, defaultAmount }: { loanId: number; defaultAmount: number }) {
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-2">
      ...
      <input className="rounded-md border border-slate-300 px-3 py-2" type="number" step="0.01" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} required />
